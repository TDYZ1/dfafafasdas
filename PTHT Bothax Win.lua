...
add_label|small| |left
add_checkbox|planting|Plant|0
add_checkbox|harvesting|Harvest|0
...
function insert()
	...
	startY = tonumber(packet:match("startY|(.*)delayPlant"))
	delayPlant = tonumber(packet:match("delayPlant|(.*)delayHarvest"))
	delayHarvest = tonumber(packet:match("delayHarvest|(.*)planting"))
end
...
if types == 2 then
	...
	if packet:find("planting|1") then
		content = [[
set_default_color|`o
